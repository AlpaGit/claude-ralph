...
export class DiscoveryAgent {
  ...
  async refreshStackProfile(args: {
    ...
    const prompt = prompts.render(PROMPT_NAMES.STACK_REFRESH, {
      normalizedPath,
  ...
  async startDiscovery(args: StartDiscoveryArgs): Promise<DiscoveryOutput> {
    ...
    const prompt = prompts.render(PROMPT_NAMES.DISCOVERY_START, {
      seedSentence: args.seedSentence,
      additionalContext: args.additionalContext || "",
  ...
  async continueDiscovery(args: ContinueDiscoveryArgs): Promise<DiscoveryOutput> {
    ...
    const prompt = prompts.render(PROMPT_NAMES.DISCOVERY_CONTINUE, {
      seedSentence: args.seedSentence,
      additionalContext: args.additionalContext || "",
      projectPath: args.projectPath || "",
      formattedAnswerHistory: formatAnswers(args.answerHistory),
      formattedLatestAnswers: formatAnswers(args.latestAnswers)
    });
  ...
  async inferStack(input: InferStackInput): Promise<InferStackResult> {
    ...
    const prompt =
      input.projectMode === "existing"
        ? prompts.render(PROMPT_NAMES.INFER_STACK_EXISTING, {
            normalizedPath: normalizedPath || "",
            projectGoal: input.projectGoal,
            constraints: input.constraints || "",
            currentStack: input.currentStack || ""
          })
        : prompts.render(PROMPT_NAMES.INFER_STACK_NEW, {
            projectGoal: input.projectGoal,
            constraints: input.constraints || "",
            currentStack: input.currentStack || ""
          });
      ...
      options: {
        ...
        agents: {
          "stack-architect": {
            ...
            prompt: prompts.render(PROMPT_NAMES.SUBAGENT_STACK_ARCHITECT, {})
          }
        }
      }
    })) {
  ...
  private async planDynamicDiscoveryJobs(input: {
    ...
    const plannerPrompt = prompts.render(PROMPT_NAMES.DISCOVERY_ORCHESTRATOR, {
      discoveryContext: input.prompt,
      projectPath: projectPath || "",
      hasProjectPath,
      stackCacheSummary,
      includeStackSpecialist: input.includeStackSpecialist,
      minAgents: MIN_DYNAMIC_DISCOVERY_AGENTS,
      maxAgents: MAX_DYNAMIC_DISCOVERY_AGENTS
    });
  ...
  private async runDiscoveryPrompt(
    ...
    const synthesisPrompt = prompts.render(PROMPT_NAMES.DISCOVERY_SYNTHESIS, {
      discoveryContext: prompt,
      specialistSummary,
      failedSpecialistSummary
    });
  ...
  private async runSpecialistAnalysis(input: {
    ...
    const specialistPrompt = prompts.render(PROMPT_NAMES.SPECIALIST_ANALYSIS, {
      jobId: input.job.id,
      discoveryContext: input.prompt,
